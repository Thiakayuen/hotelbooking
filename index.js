...
app.get("/", (req, res) => {
	if (req.session.user) {
        ...
        if (roomsSelectError) {
            console.error("Error selecting rooms:", roomsSelectError);
            res.status(500).render("500.ejs");
        } else {
            dbConnection.query(
                "SELECT * FROM spots",
                (spotsSelectError, spots) => {
                    if (spotsSelectError) {
                        console.error(
                            "Error selecting spots:",
                            spotsSelectError,
                        );
                        res.status(500).render("500.ejs");
                    } else {
...
app.post("/completeBooking", (req, res) => {
	const { id, type, client, number, checkin } = req.body;

	if (type === "room") {
		const query = `INSERT INTO roomBookings (room, client_id, number_of_nights, checkin_date) VALUES (?, ?, ?, ?)`;
		dbConnection.query(query, [id, client, number, checkin], (error) => {
			if (error) {
				console.log("Room booking error:", error);
				res.status(500).render("500.ejs");
			} else {
				res.render("bookingsuccess.ejs", { message: "Room booked successfully" });
			}
		});
	} else {
		const query = `INSERT INTO spotBookings (spot, client_id, checkin_datetime, number_of_guests, meals) VALUES (?, ?, ?, ?, ?)`;
		dbConnection.query(query, [id, client, checkin, number, "all"], (error) => {
			if (error) {
				console.log("Spot booking error:", error);
				res.status(500).render("500.ejs");
			} else {
				res.render("bookingsuccess.ejs", { message: "Spot booked successfully" });
			}
		});
	}
});
...
app.get("/confirmspotcheckin", (req, res) => {
	...
	dbConnection.query(`SELECT * FROM checkincheckoutlogs WHERE booking_id= ${idz} AND booking_type = "spot"`, (error, data) => {
		if (data.length > 0) {
			res.render("message.ejs", {message: "Client already checked in"});
		} else {
			// Insert the checkin data into the checkincheckoutlogs table
			dbConnection.query(`INSERT INTO checkincheckoutlogs(booking_id, client_id, checkin_time, booking_type) VALUES(${id}, ${client},
				CURRENT_TIMESTAMP, "spot")`, (error) => {
					if (error) {
						res.status(500).render("500.ejs")
					} else {
						res.render("message.ejs", {message: "Client checked in successfully"})
					}
				}
			)
		}
...
app.get("/confirmroomcheckin", (req, res) => {
	const {id, client} = req.query;
	// check if the client is already checked in
	dbConnection.query(`SELECT * FROM checkincheckoutlogs WHERE booking_id= ${id} AND booking_type = "room"`, (error, data) => {
		if (data.length > 0) {
			res.render("message.ejs", {message: "Client already checked in"});
		} else {
			// Insert the checkin data into the checkincheckoutlogs table
...
app.get("/confirmroomcheckout", (req, res) => {
	...
	// check if client is checked in or not checked in
	dbConnection.query(
		`SELECT * FROM checkincheckout.ogs WHERE booking_id=${id} and booking_type = "room"`, 
		(error, data) => {
			if (error) {
				return res.status(500).render("500.ejs")
			}
			if (data.length == 0) {
				...
				dbConnection.query(
					...
					(error, data) => {
						...
						if (data.length > 0) {
							res.render("message.ejs", {message: "Client already checked out"});
						} else {
